...
var addressEl = document.querySelector('.copy-field__value');
var copyBtn = document.querySelector('.copy-field__btn');
var btnLabel = copyBtn.querySelector('span');
var full = addressEl.dataset.fullAddress;
function updateAddress() {
  addressEl.textContent = full;
  var isOverflowing = addressEl.scrollWidth > addressEl.clientWidth;
  if (isOverflowing) {
    addressEl.textContent = "".concat(full.slice(0, 6), "...").concat(full.slice(-4));
  }
}
var ro = new ResizeObserver(updateAddress);
ro.observe(addressEl.parentElement);
copyBtn.addEventListener('click', function () {
  navigator.clipboard.writeText(full).then(function () {
    btnLabel.textContent = 'Copied';
    setTimeout(function () {
      return btnLabel.textContent = 'Copy';
    }, 1000);
  });
});
function initShowMore(btn, container, hiddenClass) {
  var breakpoint = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 768;
  if (!btn || !container) return;
  var textSpan = btn.querySelector('span');
  var defaultText = textSpan ? textSpan.textContent.trim() : 'Show more';
  var hideText = 'Show less';
  var getItems = function getItems() {
    return container.querySelectorAll(".".concat(hiddenClass));
  };
  function scrollToBlock() {
    var header = document.querySelector('.header');
    var headerHeight = header ? header.offsetHeight : 0;
    var offset = 16;
    var targetTop = btn.getBoundingClientRect().top + window.scrollY - headerHeight - offset;
    window.scrollTo({
  ...
  function collapse() {
    var shouldScroll = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
  ...
  btn.addEventListener('click', function () {
    ...
    if (!isExpanded) {
      ...
      if (textSpan) textSpan.textContent = hideText;
    } else {
      collapse(true);
    }
  });
...
// Tokenomics
initShowMore(document.querySelector('.tokenomics__show-more'), document.querySelector('.tokenomics__grid'), 'token-card--hidden-mobile');

// Top-sponsors leaderboard
initShowMore(document.querySelector('.top-sponsors__show-more'), document.querySelector('.leaderboard__tbody'), 'leaderboard__tr--hidden-mobile');
...
function roadmapAnimation() {
  var _steps$at;
  var track = document.querySelector('.roadmap__track');
  if (!track) return;
  var steps = Array.from(track.querySelectorAll('.roadmap__step'));
  var lastCard = (_steps$at = steps.at(-1)) === null || _steps$at === void 0 ? void 0 : _steps$at.querySelector('.roadmap__card');
  if (!lastCard) return;
  var stepDelay = 220;
  var activeCount = 0;
  var queuedTimeouts = [];
  var ticking = false;
  var clearQueuedTimeouts = function clearQueuedTimeouts() {
    queuedTimeouts.forEach(function (timeoutId) {
      return clearTimeout(timeoutId);
    });
    queuedTimeouts = [];
  };
  var setActiveCount = function setActiveCount(nextCount) {
    if (nextCount === activeCount) return;
    clearQueuedTimeouts();
    if (nextCount > activeCount) {
      var _loop = function _loop(i) {
        var timeoutId = setTimeout(function () {
          steps[i].classList.add('roadmap__step--active');
        }, (i - activeCount) * stepDelay);
        queuedTimeouts.push(timeoutId);
      };
      for (var i = activeCount; i < nextCount; i += 1) {
        _loop(i);
      }
    } else {
      var _loop2 = function _loop2(_i) {
        var timeoutId = setTimeout(function () {
          steps[_i].classList.remove('roadmap__step--active');
        }, (activeCount - 1 - _i) * stepDelay);
        queuedTimeouts.push(timeoutId);
      };
      for (var _i = activeCount - 1; _i >= nextCount; _i -= 1) {
        _loop2(_i);
      }
    }
    activeCount = nextCount;
  };
  var updateRoadmap = function updateRoadmap() {
    var rect = track.getBoundingClientRect();
    var lastCardRect = lastCard.getBoundingClientRect();
    var viewportHeight = window.innerHeight || document.documentElement.clientHeight;
    var animationStart = viewportHeight * 0.7;
    var viewportCenter = viewportHeight / 2;
    var currentOffset = animationStart - rect.top;
    var lastCardCenter = lastCardRect.top + lastCardRect.height / 2;
    var distanceToFinish = lastCardCenter - viewportCenter;
    var animationDistance = Math.max(currentOffset + distanceToFinish, 1);
    var progress = distanceToFinish <= 0 ? 1 : currentOffset / animationDistance;
    var normalizedProgress = Math.max(0, Math.min(1, progress));
    var nextCount = Math.max(0, Math.min(steps.length, Math.ceil(normalizedProgress * steps.length)));
...
;// ./src/js/components/tokenomics.js
function tokenomics() {
  var block = document.querySelector('.tokenomics');
  if (!block) return;
  var cards = Array.from(block.querySelectorAll('.token-card[data-token-key]'));
  var segments = Array.from(block.querySelectorAll('.tokenomics__segment[data-token-key]'));
  var segmentFaces = Array.from(block.querySelectorAll('.tokenomics__segment-face[data-token-key]'));
  var allSegmentParts = [].concat(segments, segmentFaces);
  if (!cards.length || !segments.length || !allSegmentParts.length) return;
  var cardsByKey = new Map();
  var segmentsByKey = new Map();
  cards.forEach(function (card) {
    var key = card.dataset.tokenKey;
    if (!cardsByKey.has(key)) cardsByKey.set(key, []);
    cardsByKey.get(key).push(card);
  });
  allSegmentParts.forEach(function (segment) {
    var key = segment.dataset.tokenKey;
  ...
  function resetState() {
    ...
    allSegmentParts.forEach(function (segment) {
      return segment.classList.remove('is-active');
    });
    block.classList.remove('has-active');
  }
  function setActive(key) {
    var activeCards = cardsByKey.get(key) || [];
    var activeSegments = segmentsByKey.get(key) || [];
    cards.forEach(function (card) {
      card.classList.toggle('is-active', activeCards.includes(card));
    });
    allSegmentParts.forEach(function (segment) {
      segment.classList.toggle('is-active', activeSegments.includes(segment));
    });
    block.classList.toggle('has-active', activeSegments.length > 0);
  }
  function bindHover(list) {
    list.forEach(function (item) {
      var key = item.dataset.tokenKey;
      if (!key) return;
      item.addEventListener('mouseenter', function () {
        return setActive(key);
      });
      item.addEventListener('mouseleave', resetState);
    });
  }
  bindHover(cards);
  bindHover(segments);
...
/******/ })()
;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,